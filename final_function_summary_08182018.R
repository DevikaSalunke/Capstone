...
load_file_to_sql = function(bedfile,cosmicfile,dblocation_and_name){
  ...
  dbWriteTable(conn = db, name = 'tmp_bed'
               , value = bedfile
               , sep="\t", row.names = FALSE, header = FALSE, overwrite=T)
  print(paste0('BED: ',dbGetQuery(db,"select count(*) from tmp_bed")))
  ...
  dbWriteTable(conn = db, name = 'cosmic'
               , value = cosmicfile
               , sep="\t", overwrite=T) 
  print(paste0('COSMIC: ',dbGetQuery(db,"select count(*) from cosmic")))
  #diconnect 
  dbDisconnect(db)
}
...
#input: gene name (loop throughtthe genelist)
#output:cosmic_file3
small_data_3 = function(x, dblocation_and_name){                  
  db_3 = dbConnect(SQLite(), dbname = dblocation_and_name) 
...
generate_output_files = function(totgenelist,outputpdfname,ppdf_filepath,ppdf_input_large_file,dblocation_and_name){
  ...
  print(1)  
    ...
    cosmic_file3 = small_data_3(gnnm,dblocation_and_name)
    
    if (nrow(text_file) > 0 & nrow(exon_bed_file) > 0 & nrow(cosmic_file3) > 0){
...
#  depth_issues_cosmic_total = rbind(depth_issues_cosmic_total, depth_issues_cosmic_row )
 ...
 #return(depth_cosmic_prctg_regions_all)
      ...
      if (nrow(text_file) == 0){
        print(paste0(gnnm," not in text file"))
      }
      if (nrow(exon_bed_file) == 0){
        print(paste0(gnnm," not in BED file"))
      }
      if (nrow(cosmic_file3) == 0){
        print(paste0(gnnm," not in COSMIC DB"))
      }
    }
  }
...
load_and_run_analysis('C:/Users/Boomerang/Downloads/Test5.sqlite', 
                               'C:/Users/Boomerang/Downloads/',
                               'Test.genelist.txt',
                               'graph231',
                               "LargeFile.stats.output.txt",
                               "Test.exons.bed",  
                               "CosmicMutantExport.tsv",
                               reload_files = T)

tmp <- load_and_run_analysis('C:/Users/Boomerang/Downloads/Test5.sqlite', 
                      'C:/Users/Boomerang/Downloads/',
                      'Test.genelist.txt',
                      'graph237',
                      "LargeFile.stats.output.txt",
...
load_and_run_analysis('C:/Users/Boomerang/Downloads/Test.sqlite', 
                      'C:/Users/Boomerang/Downloads/',
                      'genelist_3.txt.txt',
                      'garphs',
                      "LargeFile.stats.output.txt",
